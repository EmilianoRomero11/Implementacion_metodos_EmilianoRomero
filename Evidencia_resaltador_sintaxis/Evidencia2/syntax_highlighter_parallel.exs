...
defmodule Syntax_highlighter do
  ...
  #Obtain the list of files in the folder and launch a task for each one of them
  defp run_parallel(path) do
    {:ok, files} =
      ...
      files|>Enum.map(&Task.async(fn -> write_file(path,&1) end))|>Enum.map(&Task.await(&1, :infinity))
    end

    #Call the timer function with the function to start the program to obtain the execution time
    def time_parallel(path) do
      {time, _result} = :timer.tc(fn -> run_parallel(path) end)
      IO.inspect(time/1000000)
    end
  end

[path] = System.argv()
Syntax_highlighter.time_parallel(path)
